...
def test_item_activity_keeps_only_latest_cumulative_counters():
    clear_run_live_activity(126)
    for index in range(1000):
        set_run_item_activity(
            126,
            BorgItemActivity(added=index, modified=index // 10),
            timestamp=float(index),
        )
    latest = get_run_item_activity(126)
    assert latest is not None
    assert latest["added"] == 999
    assert latest["modified"] == 99
    clear_run_live_activity(126)
    assert get_run_item_activity(126) is None
