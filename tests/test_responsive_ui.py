...
def test_repository_ids_and_compact_columns_do_not_compete_with_actions():
    ...
    assert '<th>Status / ID</th><th>Belegung</th><th>Repository</th>' in javascript
    assert 'data-label="Status / ID" class="repo-status-cell"' in javascript
    assert 'title="Manager-Repository-ID">#${repo.id}' in javascript
    assert ".repositories-table th:nth-child(1) { width: 10%; }" in css
    assert ".repositories-table th:nth-child(2) { width: 17%; }" in css
    assert ".repositories-table th:nth-child(6) { width: 14%; }" in css
    assert ".repositories-table th:nth-child(7) { width: 18%; }" in css
