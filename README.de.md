...
- der aktuelle Standard-Cookiename lautet `bbm_session_v2`; individuell konfigurierte gültige Cookie-Namen bleiben möglich
...
Bei einem unterstützten Update ab der v1.3.5-Baseline bleiben externe Repository-Konfigurationen erhalten.
...
Der Docker-Bind-Mount für `/repositories` verwendet `rslave`, damit nachträglich auf dem Linux-Host eingehängte Unter-Mounts in den laufenden Container propagiert werden. Die Host-Dateisystemstruktur muss Linux-Mount-Propagation unterstützen. Nach Unmount-/Remount-Vorgängen werden propagierte Unter-Mounts ohne Container-Neustart sichtbar; anschließend den Repository-Status neu laden.
...
Die Jobübersicht zeigt zusätzlich eine Quellenstatistik mit Originalgröße und Dateianzahl. Nach einem erfolgreichen oder mit Warnung abgeschlossenen Backup werden die Werte direkt aus Borgs Abschlussstatistik übernommen. Über **Aktualisieren** beziehungsweise **Quellenstatistik** kann ein repositoryunabhängiger Live-Scan auf dem Quellgerät gestartet werden. Der bevorzugte Scanner berücksichtigt die im Job hinterlegten Borg-Ausschlussmuster sowie „Cache-Verzeichnisse ausschließen“, `nodump` und „Nur jeweiliges Quelldateisystem“. Pfadbasierte Ausschlüsse werden vor der Metadatenabfrage geprüft, sodass ausgeschlossene Dateien und komplette Verzeichnisbäume möglichst ohne unnötige `stat()`-Zugriffe übersprungen werden. Er schreibt kein Archiv. Normale erfolgreiche Scans zeigen nur Herkunft und Zeitpunkt. Kann ein Ausschluss auf einem ungewöhnlichen Client nicht sicher nachgebildet werden, erscheint ausschließlich eine konkrete Einschränkungsursache. Eingehängte Unterverzeichnisse werden mitgezählt, wenn **Nur jeweiliges Quelldateisystem** deaktiviert ist. Ist die Option aktiv, werden erkannte Unter-Mounts wie beim eigentlichen Borg-Backup übersprungen und im Scan-Protokoll ausdrücklich genannt. Nach dem nächsten abgeschlossenen Backup werden die Werte wieder durch Borgs exakte Statistik nach Anwendung der Ausschlüsse ersetzt.
...
Damit entsteht beispielsweise `bbm-12-server01-2026-07-17T22:00:00`. Verwendete Job-IDs werden auch nach dem Löschen dauerhaft reserviert und nicht erneut vergeben. Ein Job kann mehrere dauerhaft gespeicherte Archivserien besitzen; vorhandene zugeordnete Archive bleiben wiederherstellbar und werden bei der Aufbewahrung berücksichtigt. Die Vorlage muss `{now...}` oder `{utcnow...}` enthalten.
...
Der BorgBackup Manager verwendet zwei getrennte Sicherungstypen. Dadurch bleibt die eigentliche Managersicherung klein, während große Borg-Caches unabhängig davon gesichert und wiederhergestellt werden können.
...
Ein **Manager-Backup** wird über **Manager-Backup wiederherstellen** vollständig eingespielt. Vorher erzeugt BBM ein separates verschlüsseltes Sicherheitsbackup. Cache-Backups können dort nicht ausgewählt beziehungsweise nicht als Managerzustand eingespielt werden. Unterstützt werden ausschließlich Manager-Backups mit Metadatenversion v1.3.5 oder neuer.
...
Es stehen zwei getrennte Installationswege zur Verfügung.

### Geführter lokaler Quellcode-Build
...
Das ZIP erzeugt direkt `/opt/BorgBackup-Manager` und keinen Ordner mit Versionssuffix. `install.sh` erstellt die Hostkonfiguration, baut das Image lokal und startet den Stack.

### Installation mit dem veröffentlichten GHCR-Image

Für eine Installation ohne Projektquellcode sind ausschließlich die beiden Dateien im Ordner `docker-compose/` erforderlich:
...
cd /opt/borgbackup-manager
cp /pfad/BorgBackup-Manager/docker-compose/compose.yaml .
cp /pfad/BorgBackup-Manager/docker-compose/.env.example .env
chmod 600 .env
editor .env
docker compose pull
docker compose up -d
...
Bei einer echten Neuinstallation gibt der Container Benutzername und temporäres Passwort genau einmal im lokalen Startprotokoll aus. `BBM_SHOW_INITIAL_ADMIN_ON_START=1` ist im imagebasierten Beispiel voreingestellt. Die Zugangsdaten bleiben bis zum erzwungenen Passwortwechsel zusätzlich verschlüsselt abrufbar:
...
`/data/logs/debug.log` ist ausschließlich für echte Störfälle vorgesehen: unerwartete Tracebacks, unbehandelte Anwendungs- und Hintergrundfehler, kritische Framework-/Systemfehler sowie managerseitige HTTP-5xx-Antworten wie 500, 502, 503 oder 504. Normale Backup-Ausgaben, Quellenstatistik-Scans, erwartbare Borg-Warnungen und lediglich lange Antworten werden dort nicht gespeichert. Bei geschützten technischen Fehlern zeigt die WebUI eine kurze `BBM-...`-Fehler-ID. Der kurze Hinweis nach einem fehlgeschlagenen oder abgebrochenen Lauf verschwindet nach sechs Sekunden automatisch; andere handlungsrelevante rote Fehlermeldungen bleiben bis zum Schließen sichtbar.
...
Die Archivübersicht zeigt erkannte Checkpoint-Archive automatisch und kennzeichnet sie eindeutig als unvollständig. Checkpoints entstehen bei unterbrochenen Sicherungen und können nur einen Teil der vorgesehenen Dateien enthalten; Restore oder Löschen sollte daher bewusst erfolgen. Im Restore-Dialog bleibt eine separate Freigabe erhalten.
...
Nur die aktuelle Version erhält Sicherheitskorrekturen. Frühere Releases werden nicht als unterstützte Produktversionen gepflegt.
